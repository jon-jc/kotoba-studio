 ...
 * Kotoba modification: --include-pid targets a selected application's process tree.
 *
...
/* MinGW's endpointvolume.h forward-declares this SDK interface. */
#ifdef __MINGW32__
...
struct IAudioMeterInformation { const IAudioMeterInformationVtbl *lpVtbl; };
#endif
...
static BOOL g_include_process = FALSE;
    ...
    DWORD excludePid, UINT32 sampleRate, IAudioClient **outClient, const char **outErrorCode)
{
    ...
    *outErrorCode = "activation_failed";
    ...
    activationParams.ProcessLoopbackParams.ProcessLoopbackMode =
        g_include_process ? PROCESS_LOOPBACK_MODE_INCLUDE_TARGET_PROCESS_TREE
    ...
    if (SUCCEEDED(IMMDeviceCollection_GetCount(devices, &count))) {
        for (i = 0; i < count && !audible; i++) {
            ...
            if (SUCCEEDED(IMMDevice_Activate(device, &HELPER_IID_IAudioMeterInformation,
                ...
                if (SUCCEEDED(meter->lpVtbl->GetPeakValue(meter, &peak)) &&
...
static int run_capture(DWORD excludePid, UINT32 sampleRate)
{
    ...
    while (InterlockedCompareExchange(&g_running, TRUE, TRUE)) {
        ...
        if (silenceDetectionArmed &&
            ...
            if (g_include_process || !any_render_endpoint_audible(deviceEnumerator)) {
...
int main(int argc, char *argv[])
{
    ...
    for (i = 2; i < argc - 1; i++) {
        ...
        } else if (strcmp(argv[i], "--include-pid") == 0) {
            excludePid = (DWORD)strtoul(argv[++i], NULL, 10);
            g_include_process = TRUE;

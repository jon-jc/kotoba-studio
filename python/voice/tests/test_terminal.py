...
def test_japanese_output_and_state_survive_command_boundaries(tmp_path):
    ...
    # A fresh hosted Windows image can spend over 15 seconds initializing
    # Windows PowerShell. This is a correctness check, not a startup benchmark.
    result = subprocess.run(["powershell.exe", *powershell_arguments()], input=script.encode("utf-8"),
        cwd=tmp_path, capture_output=True, timeout=60, check=True, creationflags=subprocess.CREATE_NO_WINDOW)

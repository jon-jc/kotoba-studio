...
提供方下拉框优先列出已配置的提供方，同一就绪组内保留注册顺序。模型下拉框仅显示所选提供方的模型。新会话的部署默认提供方未配置时，目录加载会选择首个已配置的提供方；显式会话选择保持不变。菜单只显示模型与推理强度名称；目录中的说明仍可供其他消费方使用。`/model` 弹窗应用所选模型的默认推理强度；composer 随后可以选择任一已公布的推理强度。适配器没有推理元数据时不显示 Effort 行；不存在任意推理强度输入。

...
Web GUI 允许用户通过 `/model` 弹窗或 composer 模型控件切换既有会话使用的模型与推理强度。composer 使用独立的提供方和模型下拉框；`/model` 展示共享的按提供方分组的目录；所选模型决定可用的推理强度名称与默认值。完整选择从下一次请求开始生效；运行中的步骤保留其启动时的模型与推理强度。如果没有适配器可以服务会话路由，composer 会保持停用，直至路由恢复可用。
...
提供方下拉框仅显示已配置的提供方。需要设置或就绪状态未知的提供方仍可在模型设置中管理。模型下拉框仅显示所选提供方的模型。新会话的部署默认提供方未配置时，目录加载会选择首个已配置的提供方；显式会话选择保持不变。菜单只显示模型与推理强度名称；目录中的说明仍可供其他消费方使用。`/model` 弹窗应用所选模型的默认推理强度；composer 随后可以选择任一已公布的推理强度。适配器没有推理元数据时不显示 Effort 行；不存在任意推理强度输入。
...
输入栏通过 `data-kotoba-provider` 和 `data-kotoba-model` 属性向桌面智能体列表提供当前路由。这些属性包含提供商和模型标识，不包含凭据。各输入栏仍通过所属会话的 ModelDirectory 选择模型。

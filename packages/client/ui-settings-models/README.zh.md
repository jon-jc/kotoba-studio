---
description: "dsh Web 客户端的模型设置与产品引导插件：提供方行、API 密钥管理、模型列表与提供方中立的首次运行设置。"
...
`dsh-client-ui-settings-models` 是网页客户端的模型设置页。用户配置 API 密钥、模型列表和自定义 pi-ai 接入。页面将提供方目录、设置文档和凭据描述合并为一致快照。首次使用时按需显示提供方配置步骤；Kotoba Studio 不显示上游内测声明。
...
首次运行设置提供 OpenAI、Anthropic Claude、Kimi（Moonshot AI）、DeepSeek ；其他已安装目录中的提供方可在模型页面选择。现有适配器提供原生协议、端点和模型目录，无需网关。自定义端点仍可在模型页面中配置。
...
保留的欢迎步骤直接完成，不显示声明，也不写入确认。提供方设置读取共享设置与凭据快照：已有可用提供方时结束引导。否则，即使未挂载 DeepSeek，也可选择可写的目录路由。保存密钥会激活对应路由，并在检查就绪状态前发布已提交的设置。切换提供方会清空未保存的密钥，写入期间禁止切换。稍后配置仅结束本次引导，允许继续配置本地模型。只读或不可用的服务不会阻止进入工作空间。
...
Kotoba 保留的欢迎步骤直接完成，不显示声明，也不写入确认版本。兼容存储和设置结构保留已有数据。随后提供方步骤继续使用已有凭据编辑器。

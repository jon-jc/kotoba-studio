...
桌面版本为 **Kotoba Studio 0.7.0**，是面向 Windows x64 的未签名开发预览版。从构建输出安装 `Kotoba-Studio-0.7.0-Setup.exe`。安装包包含桌面应用、智能体运行时、音频捕获助手和 CPU 推理引擎。语音与语言模型权重单独提供。构建和验证安装包的方法见 [Windows 指南](python/voice/WINDOWS.md)。

...
Selecting Japanese presents Claude panes in Kotoba’s translated chat view automatically, once per pane and language selection. An explicit return to the terminal remains stable for sign-in and trust prompts. English selection does not change terminal views. Tests cover repeated selection, pane changes and manual returns; the provider terminal itself is not relabeled or translated.
